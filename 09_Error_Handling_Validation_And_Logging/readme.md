...
В бэкенд-приложениях различают два основных типа ошибок: _операционные ошибки (operational errors)_ и _ошибки программиста (programmer errors)_. Это разделение критично для правильной обработки исключительных ситуаций.
...
_Программные ошибки_ - это баги в коде, которые не должны возникать при правильной реализации. Это логические ошибки, которые можно исправить только изменением исходного кода.
...
Когда происходит ошибка программиста, приложение может находиться в неконсистентном состоянии. В таких случаях рекомендуется аварийно завершить процесс и перезапустить его, чтобы вернуть систему в стабильное состояние.
...
Создание собственных классов ошибок позволяет добавить контекст, код ошибки и другую полезную информацию, которая помогает при обработке и логировании.
...
Вместо обработки ошибок в каждом маршруте рекомендуется реализовать глобальный обработчик ошибок, способный обрабатывать все исключения, возникающие в приложении, в зависимости от их типа.
...
В асинхронном коде ошибки могут не быть перехвачены стандартными средствами, поскольку они возникают внутри промисов или асинхронных функций. В таких случаях исключение не передается в глобальный обработчик, если явно не вызвать `next(error)`.
...
Для упрощения валидации данных можно использовать специализированные библиотеки, например `express-validator`. Это популярный инструмент для Express-приложений, который позволяет задавать правила валидации в декларативной и читаемой форме.
...
- _Joi_ - мощная и гибкая библиотека с отличной поддержкой TypeScript, часто используется для валидации схем на уровне приложений.
...
_Логирование_ — это процесс записи информации о событиях в приложении. Хорошее логирование — необходимость для production приложений.
...
_Winston_ — одна из наиболее популярных библиотек логирования для Node.js. Она предоставляет гибкую систему с поддержкой разных уровней логирования и транспортов (места, куда отправляются логи).
...
_Sentry_ — это сервис для отслеживания ошибок в реальном времени. Когда происходит необработанная ошибка, Sentry автоматически отправляет информацию на свой сервер, где вы можете видеть стек вызовов, информацию об окружении и историю ошибок.
...
## Организация кода для обработки ошибок
