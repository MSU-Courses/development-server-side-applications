...
- _Клиент_ – это обычно веб-браузер или мобильное приложение, которое пользователь использует для доступа к веб-сайту или сервису. Клиент отправляет запросы на сервер и отображает полученные ответы (например, HTML-страницы, изображения, данные).
- _Сервер_ – это мощный компьютер или группа компьютеров, которые обрабатывают запросы от клиентов, выполняют необходимую работу (например, поиск данных, выполнение логики) и отправляют обратно ответы. Сервер может быть физическим устройством или виртуальным (облачным).
...
Ранее, в курсе по веб-разработке, вы могли создавать приложение, где фронтенд и бэкенд фактически не разделены – например, сервер (PHP) генерирует HTML-страницы на лету и отсылает их как ответ. Это характерно для _монолитной архитектуры_, когда весь веб-сайт – единое приложение: серверная логика тесно связана с презентацией.

В монолитных приложениях фронтенд и бэкенд тесно связаны: изменение в одном компоненте зачастую требует корректировок в другом, а весь код развёртывается как единое целое. Такой подход прост в реализации, его легко понять и именно с него многие начинают знакомство с веб-разработкой. Монолит удобен для небольших проектов, учебных задач или приложений с ограниченным функционалом. Однако у монолитных приложений есть и недостатки: сложности с масштабированием, трудности в поддержке и обновлении, а также ограниченная гибкость при использовании разных технологий для фронтенда и бэкенда.

### Разделение бэкенда и фронтенда. Headless архитектура
...
![Рисунок 1. Взаимодействие клиентов и сервера в headless-архитектуре](./images/01/client_server.png)

### Сервисная и микросервисная архитектура

В дополнение к разделению фронтенда и бэкенда, в современных веб-приложениях часто применяется концепция _сервисной_ или _микросервисной_ архитектуры бэкенда. В случае описанном выше, весь бэкенд реализован как одно приложение (монолитный бэкенд), и он имеет множество недостатков: сложность масштабирования, отсутствие гибкости и адаптивности, сложность внесения изменений и тестирования, увeвеличение времени развертывания по мере роста приложения, высокий риск отказа всего приложения при ошибке в одном из компонентов.

Одним из решений данной проблемы стала _сервисно-ориентированная архитектура (SOA)_. _Сервисно-ориентированная архитектура (SOA)_ - это тип архитектуры, в котором приложение разбивается на независимые сервисы, каждый из которых выполняет определенную функцию.

Проще говоря, у вас работают несколько независимых приложений (сервисов), которые взаимодействуют друг с другом через API. Каждый сервис выполняет свою функцию и может быть развернут на отдельном сервере.

![Рисунок 2. Сервисная архитектура](./images/01/01_02_soa_architecture.png)
...
## Технологии бэкенда: Node.js и альтернативы
