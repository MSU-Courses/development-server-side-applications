...
1. Выберите один тип Web-API, отличный от REST, который вы будете реализовывать в этой лабораторной работе (GraphQL, WebSockets, WebHooks, SOAP и др.).
2. В отчёте напишите не менее 1–2 абзацев:
   1. Какую конкретную задачу вы решаете (например: «_реал-тайм обновление задач между пользователями» или «централизованный API для сложных запросов_»).
   2. Почему выбранный тип API лучше подходит для этой задачи, чем REST.
