...
Получение данных в формате JSON не отличается от получения данных из формы. В обоих случаях данные попадают в `req.body`, но формат и способ отправки отличаются. 

```js
// Пример отправки JSON с клиента (например, через fetch)
fetch('/create', {
  method: 'POST',
  headers: {
  }
